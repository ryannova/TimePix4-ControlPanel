...
def Read_DACs(DAC_I, DAC_VB, DAC_VT, OSR):
    ...
    for n in range(0, len(DAC_VB)):
        ...
        print('%s\t%d\t%d\t%.3f' %(DAC_VB[n][3],dac_code,dac_code_gain,api.ADC_Sample(OSR, 0)))
    for n in range(0, len(DAC_VT)):
        api.setDacOut(DAC_VT[n][1],0)
        ...
        if (DAC_VT[n][7]):
            dac_code_gain=api.reverse_Bits(dac_code_gain,4)
        print('%s\t%d\t%d\t%.3f' %(DAC_VT[n][3],dac_code,dac_code_gain,api.ADC_Sample(OSR, 0)))

def Read_AEOC_vals(AEOC, aeoc_addr, top, OSR):
    if top:
    ...
    for n in range(0, len(AEOC)):
        ...
        api.setDacOut(selection,aeoc_addr)
        print('%s[%d]\t%.3f' %(AEOC[n][1],aeoc_addr,api.ADC_Sample(OSR, 0)))

def Read_AEOC_vals_1DAC(AEOC, DAC_num, aeoc_addr, top, OSR, print_val):
    if top:
        sel_dacout_buffer=0x1
    else:
        sel_dacout_buffer=0x3
    ...
    api.setDacOut(selection,aeoc_addr)
    adc=api.ADC_Sample(OSR, 0)
    if print_val:
        print('%s[%d]\t%.3f' %(AEOC[DAC_num][1],aeoc_addr,adc))
    return (adc)


def Set_DAC(DAC=0, val=0, sampleADC=False, OSR=0, print_enable=True):
    p_out='%s --> %d '%(DAC[3],val)
    adc_sample=0
    if len(DAC)>8: #is voltage DAC
        ...
        SimpleWriteRequest(addr=DAC[8], val=val_gain)
    SimpleWriteRequest(addr=DAC[2], val=val)
    if sampleADC:
        api.setDacOut(DAC[1],0)
        adc_sample=api.ADC_Sample(OSR, 0.0)
        p_out += ' %.6f' %adc_sample
    if print_enable:
        print(p_out)
    return adc_sample

def SupplyCenter(OSR=0):
    ...
    print("#############################################")
    for i in range(0,4):
        PS_val_ana=api.PS_monitor_center(addr_13=0x1c + i, addr_23=0x20 + i, OSR=OSR, printVal=True)
        pos+="\t%2.2f"%position[i]
        vddagnda+="\t%2.3f"%(PS_val_ana)
    for i in range(0,4):
        PS_val_dig=api.PS_monitor_center(addr_13=0x24 + i, addr_23=0x28 + i, OSR=OSR, printVal=True)
